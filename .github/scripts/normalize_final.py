...
# Remove obsolete standalone APT plugin; javac will run both Lombok and QueryDSL processors in one compilation.
...
# Configure compiler annotation processors explicitly for Java 21.
compiler=None
for pl in plugins.findall('m:plugin',N):
    a=pl.find('m:artifactId',N)
    if a is not None and a.text=='maven-compiler-plugin': compiler=pl; break
if compiler is None:
    compiler=ET.SubElement(plugins,f'{{{ns}}}plugin'); ET.SubElement(compiler,f'{{{ns}}}groupId').text='org.apache.maven.plugins'; ET.SubElement(compiler,f'{{{ns}}}artifactId').text='maven-compiler-plugin'
conf=compiler.find('m:configuration',N)
if conf is None: conf=ET.SubElement(compiler,f'{{{ns}}}configuration')
app=conf.find('m:annotationProcessorPaths',N)
if app is None: app=ET.SubElement(conf,f'{{{ns}}}annotationProcessorPaths')
...
# Keep Jakarta QueryDSL artifacts because the modernized entities use jakarta.persistence.
...
for d in deps.findall('m:dependency',N):
    ...
    if g is not None and a is not None:
        ...
        if key in [('com.querydsl','querydsl-apt'),('com.querydsl','querydsl-jpa')]:
            ...
            c.text='jakarta'
for g,a,v in [('jakarta.persistence','jakarta.persistence-api','3.1.0'),('jakarta.validation','jakarta.validation-api','3.0.2'),('org.dom4j','dom4j','2.1.4'),('org.apache.httpcomponents','httpcore','4.4.6')]:
